...
boot_alloc(uint32_t n)
{
	...
	char *result;
    uint32_t size_needed, pages_needed;
    ...
    if (ROUNDUP(n, PGSIZE) > npages_basemem * PGSIZE) {
        panic("Tried to boot_alloc but out of memory");
    }

    // Return the nextfree if there is no request
    if (n == 0) {
        return nextfree;
    }
    
    // Round n up to the nearest multiple of PGSIZE
    size_needed = ROUNDUP(n, PGSIZE);

    result = nextfree;
    nextfree += size_needed;
...
{
    if (pp->pp_ref != 0) {
        panic("There still are references to the page.");
    }

    if (pp->pp_link != NULL) {
        panic("The page is already freed.");
    }
...
{  
    ...
    for (; i < size; i += PGSIZE) {
        ...
        // Set the permission bits and the address for the virtual memory
        *page_table_entry = PTE_ADDR(pa + i) | perm;
    }
}
...
{
    pte_t* page_table_entry = pgdir_walk(pgdir, (void *) va, false);

    // Check if the page exists
    if (page_table_entry == NULL || (*page_table_entry & PTE_P) == 0) {
        return NULL;
    }

    // If pte_store is not zero, store the page table entry's address
    if (pte_store != 0) {
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
    ...
    size_t rounded_len = ROUNDUP(len, PGSIZE);

    pte_t * pte;
    for ( ; rounded_len > 0; addr += PGSIZE, rounded_len -= PGSIZE) {
        if (addr > ULIM) {
            // If the address comes from rounding what they asked for, restrict
            // to what they asked for
            if (addr > (uintptr_t) va + len) {
                addr = (uintptr_t) va + len;
            }
